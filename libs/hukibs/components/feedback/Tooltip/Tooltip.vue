...
const tooltipStyle = computed(() => ({
  ...
  '--tooltip-color': `var(--color-${props.color}, var(--color-foreground))`,
  '--tooltip-text-color': `var(--color-${props.color}-text, var(--color-background))`,
  '--tooltip-font-size': isLarge.value ? '1em' : '0.75em',
}));

const blockClasses = computed(() => {
  return bemm('',['', props.position]);
});
</script>
