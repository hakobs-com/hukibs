# Components

This index groups all component docs by category with quick links.

## UI
- [Button](/components/ui/Button)
- [ButtonGroup](/components/ui/ButtonGroup)
- [Icon](/components/ui/Icon)
- [Card](/components/ui/Card)
- [Container](/components/ui/Container)
- [Badge](/components/ui/Badge)
- [Tabs](/components/ui/Tabs)
- [CircularProgress](/components/ui/CircularProgress)
- [StatsCard](/components/ui/StatsCard)
- [OptionItem](/components/ui/OptionItem)
- [Progress](/components/ui/Progress)
- [ThemeToggle](/components/ui/ThemeToggle)

## Forms
- [BaseInput](/components/forms/BaseInput)
- [TextInput](/components/forms/TextInput)
- [SearchInput](/components/forms/SearchInput)
- [SelectInput](/components/forms/SelectInput)
- [TInputTextArea](/components/forms/TInputTextArea)

## Feedback
- [Popup](/components/feedback/Popup)
- [PopupWrapper](/components/feedback/PopupWrapper)
- [Toast](/components/feedback/Toast)
- [Tooltip](/components/feedback/Tooltip)
- [Alert](/components/feedback/Alert)
- [EmptyState](/components/feedback/EmptyState)

## Layout
- [Section](/components/layout/Section)
- [PageHeader](/components/layout/PageHeader)
- [AppHeader](/components/layout/AppHeader)
- [AppSidebar](/components/layout/AppSidebar)
- [SettingsLayout](/components/layout/SettingsLayout)

## Tables
- [Table (UI)](/components/ui/Table)
- [TableRow](/components/ui/TableRow)
- [TableCell](/components/ui/TableCell)
- [Table (Layout)](/components/layout/Table)

## Context
- [ContextMenu](/components/ui/ContextMenu)
- [ContextPanel](/components/ui/ContextPanel)
- [ContextMenuItems](/components/ui/ContextMenuItems)
